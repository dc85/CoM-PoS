...
class PDF extends FPDF {
    ...
    function load_ind_totals() {
 		...
 		foreach($staff as $s) {
 			$temp = 0.0;
 			//$date=$year . "-" . $month . "-" . $i;
 			$arg = "SELECT tsXIn,tsXOut FROM pos.tblTimeSheet WHERE tsStaff='$s[1]' AND tsDelStatus=1;";
 			//echo $arg . "<br>";
 			$rlist = mysql_query($arg);
 			while($row = mysql_fetch_row($rlist)) {
 				//echo $row[0];
 				...
 				$x = (strtotime($row[1]) - strtotime($row[0]))/60/60;
 				$temp = $temp + $x;
 				//echo $row[0];
 				//echo "<br>" . $temp . "<br>";
 				//echo "break<br>";
    ...
    function load_totals($year,$month,$days) {
 		...
 		for($i=1;$i<=$days;$i++) {
 			$temp = 0.0;
 			$date=$year . "-" . $month . "-" . $i;
 			$arg = "SELECT tsXIn,tsXOut FROM pos.tblTimeSheet WHERE tsDate='$date' AND tsDelStatus=0;";
 			//echo $arg . "<br>";
 			$rlist = mysql_query($arg);
 			while($row = mysql_fetch_row($rlist)) {
 				//echo $row[0];
    ...
    function LoadStaffSchedule($staff,$year,$month,$days) {
		...
		for($i=1;$i<=$days;$i++) {
			...
			WHERE tsStaff='$id' AND tsDate='$date' AND tsDelStatus='1';";
			//print $arg . "<br>";
			$rlist = mysql_query($arg);
