	...
	public function check_status()
		{
			if(empty($_SESSION['exp_user']) || @$_SESSION['exp_user']['expires'] < time())
				...
				{
					return $_SESSION['exp_user']['sLevel'];
					//return true;
				}
		}
	...
	public function	form()
		{
			...
			$htmlForm .= ':</label>'.
						 ...
						 '&nbsp;'.
						 '<input type="button" class="buttonfield" onclick="email();" value="'.$ACL_LANG['EMAIL'].'" />'.
						 '</form>';
			return $htmlForm;
		}
		
	public function signin($u,$p,$e)
		{
							...
							switch(LOGIN_METHOD)
								{
									...
									case 'email':
										...
										break;
									case 'old':
										$sql .= "sCNumber='".mysql_real_escape_string($u)."'";
										break;
									default:
							...
							if(mysql_num_rows($rs))
								{
									...
									//$return = true;
									$this->set_session(array_merge($row,array('expires'=>time()+(45*60),'usrlevel'=>5,'localSID'=>$e)));
									return "5";
								}
							...
							if(mysql_num_rows($rs))
								{
									...
									//$this->set_session(array_merge(mysql_fetch_assoc($rs),array('expires'=>time()+(45*60))));
									//$return = true;
									$this->set_session(array_merge($row,array('expires'=>time()+(45*60),'usrlevel'=>$row['sLevel'],'localSID'=>$e)));
									return $row['sLevel'];
								}
							mysql_free_result($rs);
							mysql_close($this->db);
