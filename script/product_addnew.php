...
function checkProductTable() {
	...
	"pBSize","pCSize","pPicture");
	$varchars = array("pName","pDescription","pUPC",
	"pSku","pProdCode","pSize","pPromo","pPicture");
	...
	if($qry_result = @mysql_query($tblProduct_qry)) {
		if(@mysql_num_rows($qry_result) == 0) {
			...
			$tblProduct_insert = "INSERT INTO pos.tblProduct(pSupplier,pCatType,pName,pDescription,pUPC,pSku,pProdCode,pBSize,pCSize,pPicture,pCrtDate,pModDate,pIsActive) VALUES(";
			foreach($tblcols as $col) {
			...
			if(@mysql_query($tblProduct_insert)) {
				...
				if($qry_result = @mysql_query($tblProduct_qry)) {
					...
					print "ERROR#2: checkProductTable() - php new product.\n";
					print (mysql_error())."\n";
				}
			} else {
				print "ERROR#3: checkProductTable() - php new product.\n";
				print "SQL: ".$tblProduct_insert."\n";
				print (mysql_error())."\n";
			}
		} else {
